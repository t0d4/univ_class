...
for t in countup(0.0, 0.40, 0.10):
    echo fmt" (t={t.formatFloat(ffDecimal, digits_under_point)}) y={normal(t).formatFloat(ffDecimal, digits_under_point)}"
...
for delta_t in [0.10, 0.050]:
    ...
    for t in countup(0.0, 0.40, delta_t):
      if toInt(100*t) mod 10 == 0:  # When t is a multiple of 0.1
        echo fmt" (t={t.formatFloat(ffDecimal, digits_under_point)}) y={y.formatFloat(ffDecimal, digits_under_point)}, v={v.formatFloat(ffDecimal, digits_under_point)}"
...
for t in countup(0.10, 0.40, 0.10):
    echo fmt" (t={t.formatFloat(ffDecimal, digits_under_point)}) y={y.formatFloat(ffDecimal, digits_under_point)}"
